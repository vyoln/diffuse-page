...
self.addEventListener("install", (event) => {
  ...
  const promise = fetch("tree.json").then((response) => response.json()).then((tree) => {
...
self.addEventListener("fetch", (event) => {
  const isInternal = !!event.request.url.match(new RegExp("^" + self.location.origin));
  if (event.request.url.includes("?ping=1")) {
    ...
    if (url.href.startsWith(GOOGLE_DRIVE))
...
function cacheThenNetwork(event) {
  const url = new URL(event.request.url);
  url.search = "";
  return caches.open(KEY).then((cache) => cache.match(url)).then((match) => match || fetch(url));
}
...
function newRequestWithAuth(event, urlWithoutToken, authToken) {
  ...
  const makeFetch = () => fetch(newRequest).then((r) => {
    ...
    retries++;
    if (retries <= 1e3)
      return new Promise((resolve, reject) => setTimeout(makeFetch().then(resolve, reject), 1e3));
    else
      throw new Error(err);
  });
  event.respondWith(
