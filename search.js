...
import_lunr.default.Pipeline.registerFunction(
...
function performSearch(rawSearchTerm) {
  let results = [];
  const searchTerm = rawSearchTerm.replace(/-\s+/g, "-").replace(/\+\s+/g, "+").split(/ +/).reduce(
  ...
  const searchTermWithAsteriks = searchTerm.split(" ").map((s) => {
    if (s.startsWith("-"))
      return s;
    return s + "*";
  }).join(" ");
  if (index) {
    results = index.search(searchTerm).map((s) => s.ref).concat(
      index.search(searchTermWithAsteriks).map((s) => s.ref)
    );
  }
  self.postMessage({
...
function updateSearchIndex(input) {
  const tracks = typeof input == "string" ? JSON.parse(input) : input;
  index = customLunr(function() {
    FIELDS.forEach(
      (field) => this.field(field)
    );
    (tracks || []).map(mapTrack).forEach((t) => this.add(t));
  });
}
function customLunr(config) {
  const builder = new import_lunr.default.Builder();
  builder.pipeline.add(removeParenthesesFromToken, import_lunr.default.stemmer);
  builder.searchPipeline.add(removeParenthesesFromToken, import_lunr.default.stemmer);
  config.call(builder, builder);
  return builder.build();
}
